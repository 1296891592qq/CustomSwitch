...
- (void)setupUI
{
    self.shape = ZTKColorSwitchShapeOval;
...
- (void)setShape:(ZTColorSwitchShape)newShape
{
    if (_shape != newShape)
        _shape = newShape;
    
    if (newShape == ZTKColorSwitchShapeOval)
    {
        [self.onBackgroundView.layer setCornerRadius:self.frame.size.height/2];
        [self.offBackgroundView.layer setCornerRadius:self.frame.size.height/2];
        [self.thumbView.layer setCornerRadius:(self.frame.size.height-kHorizontalAdjustment)/2];
    }
    else if (newShape == ZTKColorSwitchShapeRectangle)
    {
    ...
    else if (newShape == ZTKColorSwitchShapeRectangleNoCorner)
    {
